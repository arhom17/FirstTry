...
longBallClean %>% filter(LongBallTimeStamp <= 95) %>% gf_point(LongBall~LongBallTimeStamp, color =~ LongBallSuccess) + scale_color_continuous(low = "blue", high = "red")
```
...
shotRaw %>% filter(ShotTimeStamp <= 95) %>% gf_point(Shot~ShotTimeStamp, color =~ ShotFrame) + scale_color_continuous(low = "blue", high = "red")
```
